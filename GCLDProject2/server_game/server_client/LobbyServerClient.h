...
public:
	...
	void HandleGetCountryInfo( NetPack &packet );
	void HandleUpdateCrossWarInfo( NetPack &packet );
	void HandleWarStart( NetPack &packet );
	void HandleWarState( NetPack &packet );

	void HandleDevideCrossServer( NetPack &packet );
	void HandleAskCrossInfoRet( NetPack &packet );

	void SendOfficeInfoToCross();
	void AskCrossWarInfoFromLobby();
};
