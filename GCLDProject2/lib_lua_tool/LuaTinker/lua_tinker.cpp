...
//{
//	size_t len =0 ;
//	const char *pChar =lua_tolstring(L, index, &len);
//
//	// 没办法，找到正确的放回方式
//	return std::string( pChar, len ) ;
//
//}
//
//template<>
...
template<>
google::protobuf::Message* lua_tinker::read(lua_State *L, int index)
{
	lua_pbmsg* luamsg = (lua_pbmsg*)luaL_checkudata(L, index, PB_MESSAGE_META);
	google::protobuf::Message* message = luamsg->msg;
	return message ;
}

template<>
const google::protobuf::Message* lua_tinker::read(lua_State *L, int index)
{
	lua_pbmsg* luamsg = (lua_pbmsg*)luaL_checkudata(L, index, PB_MESSAGE_META);
	google::protobuf::Message* message = luamsg->msg;
	return message ;
...
template<>
void lua_tinker::push(lua_State *L, google::protobuf::Message *ret)
{
	lua_pbmsg* tmp = 
...
template<>
void lua_tinker::push(lua_State *L, const google::protobuf::Message *ret)
{
	lua_pbmsg* tmp = 
			static_cast<lua_pbmsg*>(lua_newuserdata(L, sizeof(lua_pbmsg)));
	if (tmp == NULL) 
	...
	tmp->msg = const_cast<google::protobuf::Message*>( ret );
	tmp->isDelete = false;
