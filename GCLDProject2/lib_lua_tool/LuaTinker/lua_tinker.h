...
namespace lua_tinker
{
	...
	template<>  google::protobuf::Message* read(lua_State *L, int index);
	template<>  const google::protobuf::Message* read(lua_State *L, int index);
	...
	template<> void push(lua_State *L, google::protobuf::Message *ret);
	template<> void push(lua_State *L, const google::protobuf::Message *ret);
    ...
    template<typename RVal, typename T1>
    RVal call(lua_State* L, const char* name, const T1 &arg)
    {
        lua_pushcclosure(L, on_error, 0);
        int errfunc = lua_gettop(L);
    ...
    RVal call(lua_State* L, const char* name, T1 arg1, T2 arg2)
    {
        ...
        if(lua_isfunction(L,-1))
        {
            push(L, arg1);
            push(L, arg2);
            int ret =lua_pcall(L, 2, 1, errfunc);
			if( ret != 0 )
			{
				int t = lua_type(L, -1);  
				const char* err = lua_tostring(L,-1);
...
#define LUA_CLASS_FUN_DEF( L, class_name, mem_fun ) \
    lua_tinker::class_def<class_name>( L, #mem_fun, &class_name::mem_fun )

#define LUA_CLASS_MEM_DEF( L, class_name, member ) \
    lua_tinker::class_mem<class_name>( L, #member, &class_name::member )

#define LUA_CLASS_DEF( L, class_name) \
    lua_tinker::class_add<class_name>( L, #class_name ) ;
