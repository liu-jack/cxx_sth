...
static int pb_repeated_add(lua_State* L)
{
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_INT64)
    {
		...
		if(lua_isnumber(L,2))
		{
			val = (long long)lua_tonumber(L, 2);
		}
		else
		{
			val = *(long long*)lua_touserdata(L, 2);
		}
        reflection->AddInt64(message, field, val);
		// end
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_UINT64)
    {
		...
		if(lua_isnumber(L,3))
		{
			val = (unsigned long long)lua_tonumber(L, 2);
		}
		else
		{
			val = *(unsigned long long*)lua_touserdata(L, 2);
		}
        reflection->AddUInt64(message, field, val);
		// end
    }
...
static int pb_repeated_get(lua_State* L)
{
	...
	{
		*(long long*)lua_newuserdata(L, sizeof(long long)) = reflection->GetRepeatedInt64(*message, field, index) ;
	...
	{
		*(unsigned long long*)lua_newuserdata(L, sizeof(unsigned long long)) = reflection->GetRepeatedUInt64(*message, field, index);
...
static int pb_repeated_set(lua_State* L)
{
	...
	else if(field->type() == google::protobuf::FieldDescriptor::TYPE_INT64)
	{
		...
		if(lua_isnumber(L,3))
		{
			val = (long long)lua_tonumber(L, 3);
		}
		else
		{
			val = *(long long*)lua_touserdata(L, 3);
		}
	...
	else if(field->type() == google::protobuf::FieldDescriptor::TYPE_UINT64)
	{
		...
		if(lua_isnumber(L,3))
		{
			val = (unsigned long long)lua_tonumber(L, 3);
		}
		else
		{
			val = *(unsigned long long*)lua_touserdata(L, 3);
		}
...
google::protobuf::Message* luapb_new_message( const char *pTypeName )
{
	google::protobuf::Message* message = sProtoImporter.createDynamicMessage( pTypeName );
	return message ;
}
...
static int pb_get(lua_State* L)
{
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_INT64)
    {
		...
		// else
		// lua < 5.3
		*(long long*)lua_newuserdata(L, sizeof(long long)) = reflection->GetInt64(*message, field);
		lua_getglobal(L, "__s64");
		lua_setmetatable(L, -2);
		// end
    }
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_UINT64)
    {
		...
		// else
		// lua < 5.3
		*(unsigned long long*)lua_newuserdata(L, sizeof(unsigned long long)) = reflection->GetUInt64(*message, field);
		lua_getglobal(L, "__u64");
		lua_setmetatable(L, -2);
		// end
	}
	...
	{
		const EnumValueDescriptor *pDesc =reflection->GetEnum(*message, field) ;
		lua_pushnumber(L, pDesc->number());
	}
...
static int pb_set(lua_State* L)
{
    ...
    else if(field->type() == google::protobuf::FieldDescriptor::TYPE_ENUM)
    {
		...
		// 获取 EnumValueDescriptor
		const EnumValueDescriptor *pOrignValueDesc =reflection->GetEnum(*message, field) ;
		const EnumDescriptor *pEnumDesc            =pOrignValueDesc->type() ;
		const EnumValueDescriptor *pNewValueDesc   =pEnumDesc->FindValueByName( std::string( str, strlen ) ) ;
		if( pNewValueDesc != NULL )
		{
			reflection->SetEnum(message, field, pNewValueDesc);
		}
		else
		{
			luaL_argerror(L, 2, "no this value in enum");
		}
    }
    else
    {
