...
OpHandler( CMSG_SUIT_COMPLEX)
{
	...
	if(err_code == pb::ErrCommonSuccess)
	{
		...
		m_CongratulateLog->SetAchieve(time_Now,table_id);
		CongratulateLogDB::SendInfoToSelfCongratulateDb(*this,*m_CongratulateLog,time_Now,pb::DATA_UPDATE);
		...
		if(logic_proto)
		{
			...
			Send(pb::SMSG_ACHIEVE_CONGRATULATE,msg);
			Set24HoursPastedFunc(time_Now);
