...
char* MD5::PrintMD5(uchar md5Digest[16])
{
  char chBuffer[256];
  char chEach[10];
  int nCount;
...
char* MD5::MD5String(const char* szString)
{
  Init();
...
char* MD5::MD5String( const std::string &str )
{
  Init();
  Update((unsigned char*)&str[0], (unsigned int)str.size());
  Finalize();
