...
class TcpAsyncConn : public boost::enable_shared_from_this<TcpAsyncConn>
{
	...
	public:
		static TcpAsyncConn::Ptr Create( ITcpConnMgr *pMgr, boost::asio::io_service &io_service, uint64 id ) ;
		 ...
		 * 
		 */ 
		void AsSendStackPacket( NetPack &packet ) ;
		 ...
		 * 
		 */ 
		void AsSendNewPacket( NetPack *pPacket ) ;
	...
	private:
		TcpAsyncConn( ITcpConnMgr *pMgr, boost::asio::io_service &io_service, uint64 id ) ;
		...
		void AsyncSend( NetPack *pPacket ) ;
		void HandleWrite( const boost::system::error_code& error, size_t bytes_transferred, NetPack *pPacket ) ;
	...
	private:
		...
		std::auto_ptr<NetPack>       m_pRecvPack;
