...
void PlayerSession::TakerSelectPlayer( NetPack &packet )
{
	...
	if( retInfo.result() == pb::SELECT_PLAYER_SUCCESS )
	{
        ...
        if (PlayerPtr player = sPlayerPool.GetPtrByPlayerId(retInfo.player_id())) {
			...
			PlayerPtr poolPlayer(new Player());
			poolPlayer->LoadPlayerData( retInfo.player_data() );
			poolPlayer->SetAccountId(accountId);
			m_pPlayer = poolPlayer;
			sPlayerPool.Put(poolPlayer);
		...
		m_pPlayer->SendUpdatePerformanceInfo();
		if( retInfo.is_quick_login() )
		{
			// 删除玩家数据
