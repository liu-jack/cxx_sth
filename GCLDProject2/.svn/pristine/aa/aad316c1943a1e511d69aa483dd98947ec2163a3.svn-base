...
public:
	...
	// Common
public:
	typedef typename std::map< K, T >::iterator       MapItr;
	typedef typename std::map< K, T >::const_iterator MapCItr;
...
public:
	...
	virtual T* AddEntry( T* pEntry)
	{
		...
		if ( T::HasIndex() )
		{
			...
			{
				MapItr iterEntry = m_mapTable.find(pEntry->GetKey());
				if (iterEntry != m_mapTable.end())
				{
					pRet =&( iterEntry->second );
				}
			}
		}
		else
