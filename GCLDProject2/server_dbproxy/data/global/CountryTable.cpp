...
void CountryBaseTable::SaveTo(pb::CountyInfo& msg)
{
    ...
    msg.set_exp(exp);
//	level = sCountryTableMgr.GetNpcLvByLvID(npcLevelID);
    msg.set_level(level);
    ...
    for (int i = 0; i < ARRAY_SIZE(donationMoney); ++i) {
        ...
        pInfo->set_moster_level(MonsterLevel[i]);
        pInfo->set_invade_cnt(invadeCnt[i]);
        pInfo->set_today_invade_count(today_invade_count[i]);
    }
	msg.set_last_check_npc_exp(check_npc_exp_time);
	msg.set_npc_level_id(npcLevelID);
	msg.set_tuntian_level(tuntianLevel);
	msg.set_tuntian_exp(tuntianExp);
	msg.set_prosperity_degree(m_Prosperity_Degree);
}
void CountryBaseTable::LoadFrom(const pb::CountyInfo& msg)
{
    country_id = msg.id();
    exp = msg.exp();
//	level = sCountryTableMgr.GetNpcLvByLvID(msg.npc_level_id());
	npcLevelID = msg.npc_level_id();
    level = msg.level();
    donate_times = msg.donate_times();
    npc_lv = msg.country_npc_level();
    npc_exp = msg.country_npc_exp();
	tuntianLevel = msg.tuntian_level();
	tuntianExp = msg.tuntian_exp();
    ...
    for (int i = 0; i < lst.list_size(); ++i) {
        ...
        MonsterLevel[i] = info.moster_level();
        friendVal[i] = info.friend_val();
        donationMoney[i] = info.donation_money();
        invadeCnt[i] = info.invade_cnt();
        today_invade_count[i] = info.today_invade_count();
    }
	check_npc_exp_time = msg.last_check_npc_exp();
	m_Prosperity_Degree = msg.prosperity_degree();
}
