...
TcpAsyncConn::TcpAsyncConn( ITcpConnMgr *pMgr, boost::asio::io_service& io_service, uint64 id )
	:m_isGetPacket( false )
	,m_id( id )
	,m_pMgr( pMgr ) 
	,m_pPackHandler( NULL )
...
TcpAsyncConn::Ptr TcpAsyncConn::Create( ITcpConnMgr *pMgr, boost::asio::io_service &io_service, uint64 id )
{
...
void TcpAsyncConn::Start( INetPackHandler *pPackHandler )
{
	...
	// 这里之前有个报错，在远端刚连接端口后，导致无法获取得到remote_endpoint而报错,tan 2015-8-19
	{
		...
		//NLOG( "TcpAsyncConn::Start Ip:%s, Port:%d", m_ip.c_str(), m_port) ;
	}
...
void TcpAsyncConn::HandleReadHeader(const boost::system::error_code& error, size_t bytes_transferred)
{
	...
	if ( bytes_transferred != NetPack::GetHeaderSize())
	{
		WLOG( "TcpAsyncConn::HandleReadHeader() Pack HeaderSize Not Match !" ) ;
		return;
	}
	char   type       =m_recvHeader.GetPacketType() ;
	uint16 len        =m_recvHeader.GetBodySize() ;
	uint16 opCode     =m_recvHeader.GetOpcode() ;
	uint64 sessionId  =m_recvHeader.GetSessionId() ;
	//ASSERT( type >= 0 ) ;
	//ASSERT( opCode != 0 ) ;
	if (type < 0)
	{
		WLOG( "TcpAsyncConn::HandleReadHeader() PacketType Error." ) ;
		return;
	}
	else if (opCode == 0)
	{
		WLOG( "TcpAsyncConn::HandleReadHeader() Pack Opcode Is 0." ) ;
		return;
	}
	

	m_pRecvPack.reset( new NetPack( type, len, opCode, sessionId ) ) ;
	//ASSERT( len == m_pRecvPack->GetBodySize() ) ;
	if ( len != m_pRecvPack->GetBodySize())
	{
		WLOG( "TcpAsyncConn::HandleReadHeader() Packet BodySize Not Match." ) ;
		return;
	}
	...
	else
	{
		if (m_pPackHandler)
		{
			m_pPackHandler->OnRecv( this, m_pRecvPack.release());
		}
...
void TcpAsyncConn::AsSendNewPacket( NetPack *pPacket )
{
	ASSERT( pPacket != NULL ) ;
	...
	m_socket.get_io_service().post( boost::bind( &TcpAsyncConn::AsyncSend, this, pPacket ) ) ;
}



void TcpAsyncConn::AsSendStackPacket( NetPack &packet )
{
	NetPack *pPacket =new NetPack( 0, 0 ) ;
	pPacket->Swap( packet ) ;
	AsSendNewPacket( pPacket ) ;
}


void TcpAsyncConn::AsyncSend( NetPack *pPacket )
{
	...
	pPacket->SetDynamicBodySize() ;

	boost::asio::async_write
		(
		 m_socket, 
		 boost::asio::buffer( pPacket->HeaderRawBuffer(), pPacket->DynamicTotalSize() ),
		 boost::bind
		 (
...
void TcpAsyncConn::HandleWrite(const boost::system::error_code& error, size_t bytes_transferred, NetPack *pPacket )
{
	if( m_pPackHandler )
	{
		if( !DisconnectedOrHasError(error) )
		{
			ASSERT( bytes_transferred == pPacket->DynamicTotalSize() ) ;
			m_pPackHandler->OnSend( Id(), pPacket ) ;
		}
	}
